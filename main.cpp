...
int main(int ac, char **av)
{
	...
	if ((ac == 1) || std::string(av[1]) == "map")
	{
		...
		ft::Map<char, int>::iterator mitf;
		std::map<char, int>::iterator mits;

		std::cout << "ft:\n";

		for (mitf = fmap.begin() ; mitf != fmap.end() ; mitf++)
			std::cout << (*mitf).first << ": " << (*mitf).second << std::endl;

		std::cout << "\nstd:\n";

		for (mits = smap.begin() ; mits != smap.end() ; mits++)
			std::cout << mits->first << ": " << mits->second << std::endl;
		...
		for (mitf = fffmap.begin() ; mitf != fffmap.end() ; mitf++)
			std::cout << (*mitf).first << ": " << (*mitf).second << std::endl;

		std::cout << "\n\n######### --------- reverse iterator --------- #########\n\nft:\n";

		ft::Map<char, int>::reverse_iterator rmitf;
		std::map<char, int>::reverse_iterator rits;

		for (rmitf = fmap.rbegin() ; rmitf != fmap.rend() ; rmitf++)
			std::cout << (*rmitf).first << ": " << (*rmitf).second << std::endl;
		...
		fmap.erase('c');
		mitf++;mitf++;mitf++;
		ft::Map<char, int>::iterator mitf2 = fmap.begin();
		mitf2++;mitf2++;mitf2++;mitf2++;mitf2++;mitf2++;
		fmap.erase(mitf, mitf2);
		...
		mitf2 = fmap.begin();
		std::cout << "\niterator set to ft begin : " << (*mitf2).first << "\n\nAfter swap:\n\nft:\n";
